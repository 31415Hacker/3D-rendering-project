  ...
  function recurse(tris) {
    ...
    if (tris.length <= 1) {
      ...
      tris.sort((a,b)=> {
        ...
        for(let v=0;v<3;v++){
          let off=a*3*1 + mesh.vertIdx[a*3+v]*3;
        }
        let suma=[0,0,0], sumb=[0,0,0];
        for(let v=0;v<3;v++){
          for(let k=0;k<3;k++){
            suma[k]+=mesh.positions[mesh.vertIdx[a*3+v]*3+k];
            sumb[k]+=mesh.positions[mesh.vertIdx[b*3+v]*3+k];
          }
        }
        return (suma[ax]/3)-(sumb[ax]/3);
      });
...
async function main(){
  ...
  // 4) flatten triangles → RGBA32F texture (unit 0)
  const floatsPerTri   = 3*4; // tri×(vec4)
  ...
  // 6) BVH nodes → RGBA32F (unit 2), 2 pixels/node
  ...
  function render(){
    // rotate model
    const t=lastFrame*0.00, c=Math.cos(t), s=Math.sin(t);
